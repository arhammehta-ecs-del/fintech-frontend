...
const WORKFLOW_NODE_TYPE_WRAP_THRESHOLD = 34;
...
const getWorkflowTypeLabel = (workflow: {
  nodeType?: string;
}) => formatSnakeCaseLabel(workflow.nodeType || "").trim();
              ...
              <div className="divide-y divide-slate-100">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0 truncate whitespace-nowrap text-sm font-medium text-violet-700" title={workflow.alias}>{workflow.alias}</div>
                    <div
                      className="min-w-0 truncate whitespace-nowrap text-sm text-slate-700"
                    ...
                    <div className="min-w-0 text-sm text-slate-700">
                      {(() => {
                        ...
                        const nodeTypeLabel = getWorkflowTypeLabel(workflow);
                        const shouldWrapNodeType = `${nodeDisplayName}${nodeTypeLabel ? ` (${nodeTypeLabel})` : ""}`.length > WORKFLOW_NODE_TYPE_WRAP_THRESHOLD;

                        return (
                          <div className="min-w-0 space-y-1" title={`${nodeDisplayName}${nodeTypeLabel ? ` (${nodeTypeLabel})` : ""}`}>
                            <div
                              className={cn(
                                "flex min-w-0 items-start gap-x-2 gap-y-1 text-sm leading-5 text-slate-700",
                                shouldWrapNodeType ? "flex-wrap" : "flex-nowrap",
                              )}
                            >
                              {typeof workflow.levelCount === "number" ? (
                                <span className="mt-0.5 inline-flex shrink-0 rounded bg-indigo-100 px-1 py-0.5 text-[9px] font-bold tracking-wider text-indigo-700">
                                  {`L${workflow.levelCount}`}
                                </span>
                              ) : null}
                              {shouldWrapNodeType ? (
                                <div className="min-w-0 flex-1 space-y-1">
                                  <span className="block min-w-0 break-words font-medium text-slate-700">{nodeDisplayName}</span>
                                  {nodeTypeLabel ? <span className="block break-words text-slate-500">({nodeTypeLabel})</span> : null}
                                </div>
                              ) : (
                                <span className="block min-w-0 flex-1 truncate font-medium text-slate-700">
                                  {nodeDisplayName}

...
import type { DragPayload, VisibleColumn } from "@/features/corporate/types";
import { formatDisplayDate, statusColors } from "@/features/corporate/utils";
            ...
            {sub.status === "Pending" ? (
              <>
                <Button
                  ...
                  aria-label={`Approve ${sub.companyName}`}
                  onClick={(event) => {
                    event.stopPropagation();
                    onToggleActive(sub.id, true);
                  }}
                >
                ...
                <Button
                  ...
                  aria-label={`Reject ${sub.companyName}`}
                  onClick={(event) => {
                    event.stopPropagation();
                    onToggleActive(sub.id, false);
                  }}
                >
                  <X className="h-4 w-4" />
                </Button>

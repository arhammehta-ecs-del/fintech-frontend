...
export function useUserOnboardingForm({ open, onOpenChange, onSubmit, seedMember = null }: UseUserOnboardingFormOptions) {
  ...
  const autoGeneratedSelectedNodeIds = useMemo(() => {
    if (!seedMember?.accessDetails?.length) return [];

    const sourceStateByNodeId = new Map<string, { hasAutoGenerated: boolean; hasExplicit: boolean }>();

    seedMember.accessDetails.forEach((row) => {
      const nodeId = (row.nodePath || "").trim();
      if (!nodeId) return;

      const current = sourceStateByNodeId.get(nodeId) ?? { hasAutoGenerated: false, hasExplicit: false };
      const normalizedSourceTag = (row.sourceTag || "").trim().toUpperCase();

      if (normalizedSourceTag === "AUTO_GENERATED") {
        current.hasAutoGenerated = true;
      } else {
        current.hasExplicit = true;
      }

      sourceStateByNodeId.set(nodeId, current);
    });

    return Array.from(sourceStateByNodeId.entries())
      .filter(([, state]) => state.hasAutoGenerated && !state.hasExplicit)
      .map(([nodeId]) => nodeId);
  }, [seedMember]);
